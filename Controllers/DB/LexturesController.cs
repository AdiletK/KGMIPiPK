...
namespace KGMIPiPK.Controllers
{
    ...
    public class LexturesController : Controller
    {
        ...
        public async Task<IActionResult> CreateAsync([Bind("Nom,Day,Group,Tema,Teacher,Hours,VidZan")] Lextures lextures)
        {
            if (ModelState.IsValid)
            {
                //using Dapper to execute stored procedure
                int id = await _context.SP_InsertToLexturesAsync(lextures);
